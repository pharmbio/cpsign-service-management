...
RUN apt update && apt install -y --no-install-recommends nano mysql-client apt-transport-https ca-certificates gnupg curl && \
    ...
    curl -o /tmp/pachctl.deb -L https://github.com/pachyderm/pachyderm/releases/download/v${pachctl_version}/pachctl_${pachctl_version}_amd64.deb && sudo dpkg -i /tmp/pachctl.deb && \
